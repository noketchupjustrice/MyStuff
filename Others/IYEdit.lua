--New Commands, "void", "rvoid", "rkill", "rescale" | Updated Commands : "kill"
...
local function rescale()
	local a=game:GetService("Players").LocalPlayer.Character;local b=a:FindFirstChildOfClass("Humanoid")local function c()for d,e in pairs(a:GetDescendants())do if e:IsA("BasePart")and e.Name~="Head"then for d,f in pairs(e:GetDescendants())do if f:IsA("Attachment")and f:FindFirstChild("OriginalPosition")then f.OriginalPosition:Destroy()end end;e:WaitForChild("OriginalSize"):Destroy()end end end;c()b:WaitForChild("BodyTypeScale"):Destroy()c()b:WaitForChild("BodyWidthScale"):Destroy()c()b:WaitForChild("BodyDepthScale"):Destroy()c()b:WaitForChild("HeadScale"):Destroy()
end
...
CMDs[#CMDs + 1] = {NAME = '(NEW) desync [plr] / dsync [plr]', DESC = 'Desyncs a players boombox if they are holding one'}
CMDs[#CMDs + 1] = {NAME = '(NEW) loopdesync [plr] / ldsync [plr]', DESC = 'Loop desyncs a players boombox if they are holding one'}
CMDs[#CMDs + 1] = {NAME = '(NEW) unloopdesync / unldsync', DESC = 'Stops desyncing a players boombox if they are holding one'}
...
CMDs[#CMDs + 1] = {NAME = '(NEW) rescale / rscale', DESC = 'Rescales your character (R15)'}
CMDs[#CMDs + 1] = {NAME = '(UPDATED) kill [plr] (TOOL)', DESC = 'Kills a player (YOU NEED A TOOL)'}
CMDs[#CMDs + 1] = {NAME = '(NEW) rkill [plr] (TOOL)', DESC = 'Rescales your character and kills a player (YOU NEED A TOOL)'}
CMDs[#CMDs + 1] = {NAME = '(NEW) void [plr] (TOOL)', DESC = 'Voids a player (YOU NEED A TOOL AND MORE MASS'}
CMDs[#CMDs + 1] = {NAME = '(NEW) rvoid [plr] (TOOL)', DESC = 'Rescales your character and voids a player (YOU NEED A TOOL)'}
...
addcmd('desync',{'dsync'},function(args, speaker)
    local players = getPlayer(args[1], speaker)
    for i, v in pairs(players) do
        local Target = Players[v]
        for I = 1, #Target.Character:GetDescendants() do
            local Object = Target.Character:GetDescendants()[I]
...
local LoopDesync = false
addcmd('loopdesync',{'ldsync'},function(args, speaker)
    LoopDesync = true
    local players = getPlayer(args[1], speaker)
    while LoopDesync == true do
        for i, v in pairs(players) do
            local Target = Players[v]
            for I = 1, #Target.Character:GetDescendants() do
                local Object = Target.Character:GetDescendants()[I]
...
addcmd('unloopdesync',{'unldsync'},function(args, speaker)
    LoopDesync = flase
end)
...
addcmd('rescale',{'rscale'},function(args, speaker)
    rescale()
end)
...
addcmd('rkill',{'rescalekill'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
        local Target = Players[v]
        local LocalPlayer = game.Players.LocalPlayer
...
addcmd('void',{'fevoid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local Target = Players[v]
        local OldTime = tick()
		...
		LocalPlayer.Character:SetPrimaryPartCFrame(Target.Character.Head.CFrame)
        ...
        BP.Position = Vector3.new(0, -3000, 0)
		...
		coroutine.wrap(function()
			repeat
				if Target.Character.Humanoid.Health <= 0 then
					notify(Target.Name.." | has died.", "Took: "..tostring(tick() - OldTime):sub(1, 5).." Seconds")
					break
				end
				task.wait()
...
addcmd('rvoid',{'rescalevoid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local Target = Players[v]
        local OldTime = tick()
