...
local function RegisterCollisionGroup(Name)
	...
	sethiddenproperty(Workspace, "CollisionGroups", string.format("%s\\%s^%s^%s", gethiddenproperty(Workspace, "CollisionGroups"),Name,tonumber(#PhysicsService:GetRegisteredCollisionGroups()), "-1" ))
	return true	
end
...
local function UnregisterCollisionGroup(CollisionGroup)
...
xpcall(function()
	...
	old = hookmetamethod(game, "__namecall" ,newcclosure(function(self,...)
		...
		if self == PhysicsService then
			...
			elseif getnamecallmethod() == "UnregisterCollisionGroup"  then
				return	UnregisterCollisionGroup(args[1])
			elseif getnamecallmethod() == "RegisterCollisionGroup" then
				return	RegisterCollisionGroup(args[1])
